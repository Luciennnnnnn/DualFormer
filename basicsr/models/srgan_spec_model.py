...
class SRGANSpecModel(SRModel):
    ...
    def init_training_settings(self):
        ...
        if self.opt['network_d']['type'] == 'VisionTransformer':
            opt = self.opt['network_d']
            opt.pop('type')
            self.net_d = VisionTransformer(**opt)
        else:
    ...
    def optimize_parameters(self, current_iter):
        ...
        if (current_iter % self.net_d_iters == 0 and current_iter > self.net_d_init_iters):
            ...
            # gan loss
            fake_g_pred_d = self.net_d(self.output)
            fake_g_pred_spectral_d = self.net_spectral_d(self.output)
        ...
        assert torch.isnan(l_spectral_d_real).any() == False and torch.isinf(l_spectral_d_real).any() == False
        ...
        assert torch.isnan(l_spectral_d_fake).any() == False and torch.isinf(l_spectral_d_fake).any() == False
